create database store; /*crearea bazei de date*/
use store; /* selectarea bazei de date*/
create table sales /* crearea (definirea) tabelei cu diferite tipuri de date de tip string, integer si data plus 
definirea cheii principale*/
(
    ...
    create table orders /* crearea celei de a doua tebele cu care se face join. Folosirea foreign key . /* 15:21:39	create table orders /* crearea celei de a doua tebele cu care se face join. Folosirea foreign key */ 
/**Error Code: 3780. Referencing column 'CarrierTrackingNumber' and referenced column 'productId' in foreign 
key constraint 'orders_ibfk_1' are incompatible**/
(
    sales_OrderId int primary key, 
    order_quantity int,
    productId int,
    CarrierTrackingNumber varchar (20),
    foreign key (CarrierTrackingNumber) references sales (productId)
    );
    desc sales; /*desc - instructiune pentru a verifica structura tabelei*/
...
alter table sales modify product_name varchar(40); /* modificarea variabilei product_name de la 20 de la 40 */
...
use store; /* folosirea bazei de date sales*/
create table salesOrdersDetail 
(
    sales_OrderId int primary, 
    order_quantity int,
    productId,
    CarrierTrackingNumber varchar (20)
    );
/* 15:21:39	create table orders /* crearea celei de a doua tebele cu care se face join. Folosirea foreign key */ 
/**Error Code: 3780. Referencing column 'CarrierTrackingNumber' and referenced column 'productId' in foreign 
key constraint 'orders_ibfk_1' are incompatible**/
...
/*Error Code: 1062. Duplicate entry '43668' for key 'salesordersdetail.PRIMARY'	*/

alter table salesordersdetail drop primary key; /* modificarea variabilei de la cheie primara la simplu int */
desc salesordersdetail;
...
select * 
from sales as s
join salesordersdetail as sod
on s. productId = sod. productId; /* inner join intre 2 tabele dupa productId*/
